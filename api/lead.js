/*
 * Lead delivery for the DF360 microsite.
 *
 ...
 *                    set it to something like "DF360 Site <noreply@apexmd.com>".
 *
...
module.exports = async function handler(req, res) {
  ...
  const subject = kind === 'contact'
    ? 'DF360 site — contact form' + (name ? ' — ' + name : '')
    : 'DF360 site — member guide request';

  const attrRows = rows(body, ATTR_LABELS);
  const html =
    '<div style="max-width:640px;margin:0 auto;font:14px system-ui,sans-serif;color:#101010">' +
      '<p style="margin:0 0 4px;font:700 11px system-ui;letter-spacing:.14em;text-transform:uppercase;color:#DE192C">DF360 &times; Apex MD</p>' +
  ...
  const from = process.env.LEAD_FROM || 'DF360 Site <onboarding@resend.dev>';
