/* DF360 x Apex MD - tracking + lead capture wiring.
...
(function () {
  ...
  function track(name, params) {
    var payload = params || {};
    window.dataLayer.push(Object.assign({ event: name }, payload, { df360_session: stored.session_id }));
    if (window.fbq) {
  ...
  function decorate(url) {
    try {
      ...
      u.searchParams.set('df360_sid', stored.session_id);
      u.searchParams.set('src', 'df360');
      return u.toString();
    } catch (e) { return url; }
  }
  ...
  window.spgSubmitLead = function (fields, kind) {
    ...
    if (!CONFIG.LEAD_ENDPOINT) {
      console.info('[df360] LEAD_ENDPOINT not set; captured locally only:', body);
    ...
    return fetch(CONFIG.LEAD_ENDPOINT, init)
      ...
      .catch(function (err) { console.warn('[df360] lead post failed', err); return { ok: false, error: String(err) }; });
  };
